...
class RightSidebar extends Component {
	...
	render(){
		...
		var events = this.props.events.map(function(event, i){
			const rsvp = (event.status == 'open') ? <a href={'/event/'+event.slug} style={{marginRight:0}} className="button button-3d button-mini button-rounded button-teal">Attend</a> : <a href={'/event/'+event.slug} style={{marginRight:0}} className="button button-3d button-mini button-rounded button-red">Sold Out</a>
			...
			return (
				<div key={event.id} style={{border:'1px solid #ddd', background:'#f9f9f9', marginBottom:16}}>
					...
					<div style={{padding:12, height:104, textAlign:'right'}}>
						<h5 style={{fontWeight:200, marginBottom:0, fontSize:12}}><a href={'/event/'+event.slug}>{event.title}</a></h5>
						<span style={{fontWeight:100, fontSize:12}}>{event.date}, {event.time}</span><br />
						{rsvp}
					</div>
				</div>
			)
